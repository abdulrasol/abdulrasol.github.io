...
function Projects($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    for (let i = 0, $$length = each_array.length; i < $$length; i++) {
      ...
      if (project.liveUrl) {
        $$renderer2.push("<!--[-->");
        $$renderer2.push(`<a${attr("href", project.liveUrl)} class="btn btn-primary btn-sm"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg> ${escape_html(project.liveUrl && (project.liveUrl.includes("/madarik") || project.liveUrl.includes("/meterlog") || project.liveUrl.includes("/store-management")) ? t("projects.landingPage", store_get($$store_subs ??= {}, "$locale", locale)) : t("projects.liveDemo", store_get($$store_subs ??= {}, "$locale", locale)))}</a>`);
      } else {
        $$renderer2.push("<!--[!-->");
      }
      $$renderer2.push(`<!--]--> `);
