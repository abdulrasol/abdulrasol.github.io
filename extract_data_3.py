...
for app in apps:
    ...
    # Replace featuresAr and featuresEn
    js_content = re.sub(r'featuresAr:\s*\[.*?\]', f"featuresAr: [\n                {features_ar_str}\n            ]", js_content, flags=re.DOTALL)
    js_content = re.sub(r'featuresEn:\s*\[.*?\]', f"featuresEn: [\n                {features_en_str}\n            ]", js_content, flags=re.DOTALL)
    ...
    # In JS, heroTitle and heroSubtitle are defined twice (in ar and en translations)
    # So we split the translations object
    ar_idx = js_content.find("en: {")
    ...
    final_content = ar_section + en_section
